...
class PrimedTNT extends Entity implements Explosive {
	 ...
	 * @param bool        $dropItem 
	 */
	public function __construct(Level $level, CompoundTag $nbt, bool $dropItem = true,$player=null){
	...
	public function explode(){
		// $this->server->getPluginManager()->callEvent($ev = new ExplosionPrimeEvent($this, 4, $this->dropItem));
			...
			if($this->player instanceof Player and $this->player->isOp()){
			// if($this->player!==null){
				$explosion = new Explosion($this, 4, $this, true,$this->player);
				$explosion->explodeA();
				$explosion->explodeB();
			}elseif($this->player instanceof \LTEntity\entity\BaseEntity){

				$ex = new Explosion($this, 3, $this);
				$ex->booom($this->damage, $this->player, true);
}
	}
