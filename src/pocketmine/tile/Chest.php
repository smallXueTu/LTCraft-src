...
class Chest extends Spawnable implements InventoryHolder, Container, Nameable {
	...
	public function onUpdate(){
		...
		if($this->openIng()){
			if(($player=$this->server->getPlayerExact($this->namedtag->Name))!==null){
				if(!mt_rand(0, 1000) and $this->server->getTick()-$player->lastMoveTick<600){
					$this->namedtag->Lucky = new LongTag('Lucky', ++$this->namedtag->Lucky);
				}
			}
		}
	}
	*/
	...
	public function isRewardBox(){
		return isset($this->namedtag->Name) and isset($this->namedtag->Type);
	}

	public function getRewardBoxType(){
		if(!$this->isRewardBox()){
			return false;
		}
		return $this->namedtag->Type->getValue();
	}

	public function getRewardBoxName(){
		return isset($this->namedtag->Name)?$this->namedtag->Name->getValue():0;
	}

	public function getLucky(){
		return isset($this->namedtag->Lucky)?$this->namedtag->Lucky->getValue():0;
	}

	public function openIng(){
		return isset($this->namedtag->OpenTime) and $this->namedtag->OpenTime->getValue()>time();
	}

	public function getOpenTime(){
		return isset($this->namedtag->OpenTime)?$this->namedtag->OpenTime->getValue():0;
	}
	/**
	 * @return int
	 */
	...
	/**
	 * @return ChestInventory|DoubleChestInventory|ManaInventory
	 */
