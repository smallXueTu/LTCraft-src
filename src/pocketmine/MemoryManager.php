...
class MemoryManager {
	 ...
	 * @return int
	 */
	public function triggerGarbageCollector($auto=true){
		if(!$auto)return gc_collect_cycles();
		$start=microtime(true);
		if(count($this->server->getOnlinePlayers())>0){
			$this->server->getLogger()->info('§e§l开始自动保存存和回收垃圾');
			$this->server->broadcastTip('§o§c开始保存和回收垃圾，可能会卡顿....',$this->server->getOnlinePlayers(),true);
		}
		foreach($this->server->getOnlinePlayers() as $p)$p->checkNetwork();
		...
		foreach($this->server->getOnlinePlayers() as $player)$player->save();
		...
		\LTCraft\Main::save();
		if(count($this->server->getOnlinePlayers())>0){
			$this->server->broadcastTip('§o§c保存回收垃圾完成，耗时:§a'.(microtime(true)-$start).'秒',$this->server->getOnlinePlayers(),true);
			$this->server->getLogger()->info('§e§l保存成功！');
		}
	...
	public function getAllObject($from){
		$data = [];
		$refCounts = [];
		$Objects = [];
		$this->continueDump($from, $data, $Objects, $refCounts, PHP_INT_MAX, PHP_INT_MAX, PHP_INT_MAX);
		do{
			$continue = false;
			foreach($Objects as $hash => $object){
				if(isset(self::$Objects[$hash]))continue;
				if(!is_object($object)){
					self::$Objects[$hash]=[$object,is_object($from)?get_class($from):$from];
				}else{
					self::$Objects[$hash]=[$object,is_object($from)?get_class($from):$from];
					$continue = true;
					...
					foreach($reflection->getProperties() as $property){
						...
						$this->getAllObject($property->getValue($object));
					}
				}
			}
		}while($continue);
	}
	public function getMaxMemoryObject($from){
		$this->getAllObject($from);
		$tmp=null;
$MaxMemoryObject=null;
$maxMemory=0;
...
$tmp=$object[0];
			...
			$tmp=null;
       gc_collect_cycles();
			$afterMemory=$beforeMemory-memory_get_usage();
			if($afterMemory>$maxMemory){
				$maxMemory=$afterMemory;
				$MaxMemoryObject=$object[1];
			}
		}
foreach(self::$Objects as $hash => $object)unset(self::$Objects[$hash]);
		return [self::formatBytes($maxMemory), $MaxMemoryObject];
	}
