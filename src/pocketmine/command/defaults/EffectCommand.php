...
//		}
		...
		if($duration === 0){
			...
			if($player->addEffect($effect)){
				...
				if(!$player->isOp()){
if($sender instanceof Player)$o=$sender->getDisplayName();else $o='系统';
					$player->sendMessage('管理员：'.$o.'赐予你'.$effect->getName().'*'.$effect->getAmplifier().'持续：'.$effect->getDuration() / 20 .'秒的效果');
				}
			}
		}
