...
class ListCommand extends VanillaCommand {
	...
	public function execute(CommandSender $sender, $currentAlias, array $args){
		...
		$online = "";
		$onlineCount = 0;
		if(isset($args[0]) and $this->testPermission($sender)){
			foreach($sender->getServer()->getOnlinePlayers() as $player){
				if($player->isOnline() and (!($sender instanceof Player) or $sender->canSee($player))){
					$online .= $player->getName().'['.$player->getLevel()->getName().']' . ", ";
					++$onlineCount;
				}
			}
		}else{
		...
		$sender->sendMessage('当前有'.$onlineCount .'/'. $sender->getServer()->getMaxPlayers().'玩家在线');
