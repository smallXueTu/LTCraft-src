...
//		}
    ...
    private function tickChunks()
    {
        ...
        $chunksPerLoader = min(200, max(1, (int)((($v - count($this->loaders)) / count($this->loaders)) + 0.5)));
    ...
    public function getAllInBox(Vector3 $startPos, Vector3 $endPos) : array {
        ...
        $maxX = max($startPos->getX(), $endPos->getX());
     ...
     */
    public function getCollisionCubes(Entity $entity, AxisAlignedBB $bb, bool $entities = true, $block=true): array
    {
        $minX = Math::floorFloat($bb->minX);
        $minY = Math::floorFloat($bb->minY);
        $minZ = Math::floorFloat($bb->minZ);
	...
	*/
     ...
     */
    public function dropItem(Vector3 $source, Item $item, Vector3 $motion = null, int $delay = 10, $send = true)
    {
    ...
    public function useBreakOn(Vector3 $vector, Item &$item = null, Player $player = null, bool $createParticles = true, Player $p = null): bool
    {
        ...
        if ($player !== null) {
            ...
            if (!($item instanceof Mana)) {
                ...
                $breakTime -= 2; //2 tick compensation
                $enchantLevel = (int)$item->getEnchantmentLevel(Enchantment::TYPE_MINING_EFFICIENCY);
                $time = $enchantLevel == 0 ? 0 : $breakTime / $enchantLevel;
                if (!$ev->getInstaBreak() and (((ceil($player->lastBreak * 20)) + $breakTime) > (ceil(microtime(true) * 20) + $time * $enchantLevel))) {//计算是否作弊
                    $player->lastBreak = PHP_INT_MAX;
                    return false;
                }
            }
            $this->server->getPluginManager()->callEvent($ev);//调用事件
            if ($ev->isCancelled()) {
                return false;
            }
            $player->lastBreak = PHP_INT_MAX;
            ...
            if ($target->canDrop()) {
                ...
                foreach ($drops as $k => $i) {
                    if ((isset ($i[0])) && (isset ($i[1])) && (isset ($i[2]))) $drops[$k] = Item::getForString($i[0], $i[1], $i[2], $player);
                }
            } else {
                $drops = [];
            }
        ...
        */
        if ($createParticles)$this->addParticle(new DestroyBlockParticle($target, $target));

        $tile = $this->getTile($target);
        if ($tile !== null) {
            if ($tile instanceof InventoryHolder and !$tile->closed) {
        ...
        $target->onBreak($item);
        if($tile !== null and !$tile->closed)$tile->close();
        if ($item !== null) {
            $item->useOn($target);
            if ($player != null) $item->entityUseOn($target, $player);
    ...
    public function useItemOn(Vector3 $vector, Item &$item, int $face, float $fx = 0.0, float $fy = 0.0, float $fz = 0.0, Player $player = null): bool
    {
        ...
        if ($player !== null) {
            if ($item instanceof LTItem and $item->getId()==383){
                $player->attentionSend = true;
            }else{
            ...
            if (!$ev->isCancelled()) {
                ...
                if (!$player->isSneaking() or $item instanceof LTItem) {
     ...
     * @return Lightning
     */
    public function spawnLightning(Vector3 $pos, $damage = 0, Entity $owner = null): Lightning
    {
        $nbt = new CompoundTag("", [
            "Pos" => new ListTag("Pos", [
