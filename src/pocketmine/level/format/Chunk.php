...
class Chunk{
     ...
     * @return int
     */
	public function isClose(){//修复了tile不保存的一些问题#待测试
		return $this->closed;
	}
     ...
     * @param Level $level
     */
    public function initChunk(Level $level, $force=false){
        if(!$this->isInit or $force===true){
            $changed = false;
     ...
     * @return string
     */
	public static function getEmptySubChunkfastSerialize($x, $z){
		$stream = new BinaryStream();
        $stream->putInt($x);
        $stream->putInt($z);
        $stream->putByte(0);
