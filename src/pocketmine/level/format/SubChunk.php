...
class SubChunk {
     ...
     * @param string $drop
     */
	public function __construct(string $ids = "", string $data = "", string $skyLight = "", string $blockLight = "", string $drop = ""){
        ...
        self::assignData($this->drop, $drop, 4096, "1");
	}
     ...
     * @return bool
     */
	public function setBlockDrop(int $x, int $y, int $z, bool $drop) : bool {
        $this->drop[($x << 8) | ($z << 4) | $y] = $drop == true?1:0;
        return true;
    }
     ...
     * @return bool
     */
    public function getBlockDrop(int $x, int $y, int $z) : bool {
        return $this->drop[($x << 8) | ($z << 4) | $y] == 1;
    }
	 ...
	 * @return string
	 */
	public function getDropArray() : string{
		assert(strlen($this->drop) === 4096, "Wrong length of drop array, expecting 2048 bytes, got " . strlen($this->drop));
		return $this->drop;
	}
