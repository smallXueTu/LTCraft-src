...
abstract class BaseInventory implements Inventory {
	 ...
	 * @return bool|int
	 */
	public function containsLTItem(array $itemInfo){
		foreach($this->getContents() as $index=>$i){
			if($i instanceof LTItem and Main::getInstance()->isEquals($i, $itemInfo)){
			    return $index;
			}
		}
		return false;
     ...
     * @return bool
     */
	public function isNoFull(){
		for($i = 0; $i < $this->getSize(); ++$i){
			$slot = $this->getItem($i);
			if($slot->getId() === Item::AIR)return true;
		}
		return false;
	}
	/**
