...
use pocketmine\utils\AchievementAndChallenge;
...
class PlayerInventory extends BaseInventory {
			...
			if($contents instanceof ListTag){ //Saved data to be loaded into the inventory
				foreach($contents as $item){
					if($item["Slot"] >= 0 and $item["Slot"] < $this->getHotbarSize()){ //Hotbar
						...
						$it=Item::nbtDeserialize($item);
						if(!isset(Item::$list[$it->getId()]) and !($it instanceof \LTItem\LTItem))$it=Item::get(0);
						$this->setItem($item["Slot"] - $this->getHotbarSize(), $it, false);
					}
				}
			}else{
	...
	public function setInv($contents){
		...
		}else throw new \InvalidArgumentException("Expecting ListTag, received " . gettype($contents));
	...
	public function setItem($index, Item $item, $send = true){
        ...
        if (($r = AchievementAndChallenge::getTypeAndNameForItem($item))!=null){
            $this->getHolder()->newProgress($r[1], $r[2]??'', $r[0]);
        }
	 ...
	 * @param Player|Player[] $target
	 */
	public function sendNullContents($target){
		if($target instanceof Player){
			$target = [$target];
		}
