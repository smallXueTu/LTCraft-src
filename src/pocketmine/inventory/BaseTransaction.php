...
class BaseTransaction implements Transaction {
	...
	public function execute(Player $source) : bool{
		if($this->getInventory()->processSlotChange($this)){ //This means that the transaction should be handled the normal way
			if($source->getGamemode()!==1){//因为玩家死亡的时候是观察者模式 因此玩家可以在观察者模式的时候跳过这个检测 已修复
				...
				if($change["in"] instanceof Item){
					// if($this->slot>=36){
						// if(!$this->getInventory()->slotContains($this->getSlot(), $change["in"])){
							// return false;
						// }
					// }else
						// var_dump(get_class($change["in"]));
						// var_dump($source->getFloatingInventory()->contains($change["in"]));
