...
class Item implements ItemIds, \JsonSerializable {
    ...
    public static $LTIteminitd = false;
    /**
     * @param string $tag
     *
     * @return CompoundTag
     */
    public static function parseCompoundTag(string $tag) : CompoundTag{
    ...
    private static function initCreativeItems(){
        ...
        foreach($creativeItems->getAll() as $data){
            ...
            if($item->getName() === "Unknown" or $item->getId()===0){
                continue;
            }
    ...
    public static function get(int $id, int $meta = 0, int $count = 1, string $tags = "", $weaponInit = true) : Item{
        try{
            if($tags!==''){
                $tags=self::parseCompoundTag($tags);
                if(self::$LTIteminitd and Main::isThisItemForNBT($tags) and ($data=Main::getTypeAndNameForNBT($tags))!==false and ($conf=Main::getInstance()->getConf($data[0], $data[1]))!==false){
                    switch($data[0]){
                        case '近战':
                        case '远程':
                        case '通用':
                            return Weapon::getWeapon($data[0], $conf, $count, $tags, $weaponInit);
                            break;
                        case '盔甲':
                        ...
                        case '材料':
                            ...
                            break;
                        case '魔法':
                            return ManaSystem::getManaItem($data[1], $conf, $count, $tags, $weaponInit);
                            break;
                        case '饰品':
                            return new BaseOrnaments($conf, $count, $tags, $weaponInit);
                            break;
                    }
                }
            }
            ...
            if($class === null){
                ...
                if($tags=='')return new ItemBlock(new $class($meta), $meta, $count);
                return (new ItemBlock(new $class($meta), $meta, $count))->setCompoundTag($tags);
            }else{
                if($tags=='')return new $class($meta, $count);
    ...
    public static function getForString($info1, $info2, $info3, $player) : Item{
        switch($info1){
            case '近战':
            case '远程':
            case '通用':
                $weapon = Main::getInstance()->createWeapon($info1, $info2, $player);
                $weapon->setCount($info3);
                return $weapon;
                break;
            case '盔甲':
                $armor = Main::getInstance()->createArmor($info2, $player);
                $armor->setCount($info3);
                return $armor;
                break;
            case '材料':
                $material = Main::getInstance()->createMaterial($info2);
                $material->setCount($info3);
                return $material;
                break;
            case '魔法':
                $mana = Main::getInstance()->createMana($info2, $player);
                $mana->setCount($info3);
                return $mana;
                break;
            case '饰品':
                $ornaments = Main::getInstance()->createOrnaments($info2);
                $ornaments->setCount($info3);
                return $ornaments;
                break;
            default:
                return self::get($info1, $info2, $info3);
        }
    }
    ...
    public static function fromString(string $str, bool $multiple = false){
        if($multiple === true){
            ...
            if(defined(Item::class . "::" . strtoupper($b[0]))){
                ...
                $item = self::get(intval($b[0]) & 0xFFFF, $meta);
            }
            if(isset($b[2]))$item->setCustomName($b[2]);
            return $item;
        }
    }
    ...
    public function __construct(int $id, int $meta = 0, int $count = 1, string $name = "Unknown"){
        if(!isset(self::$list[$id]) and !($this instanceof LTItem) and !($this instanceof MenuItem)){
            $id=0;
            $meta=0;
            $count=0;
            $name="Unknown";
     ...
     * 清除NBT缓存
     */
    public function clearCachedNBT(){
        $this->cachedNBT = null;
    }
    ...
    public function clearEnchantments(){
        if(!$this->hasCompoundTag()){
            return true;
        }
        $tag = $this->getNamedTag();
    ...
    /**
     * @param string $name
     * @param bool $th 立即更新nbt
     * @return $this
     */
    public function setCustomName(string $name, $th = false){
        if($name === ""){
     ...
     * @return bool
     */
    public function entityUseOn(Block $block, Entity $entity){
        return false;
    }
    ...
    public function setUnableConvert($v){
        if(!$this->hasCompoundTag()){
            $tag = new CompoundTag("", []);
        }else{
            $tag = $this->getNamedTag();
        ...
        $tag->UnableConvert = new ByteTag('UnableConvert',$v==true?1:0);
        $this->setNamedTag($tag);
     ...
     * @return bool
     */
    public function isUnableConvert(){
        $tag = $this->getNamedTagEntry("UnableConvert");
    ...
    public static function nbtDeserialize(CompoundTag $tag) : Item{
        if(isset($tag->tag) and $tag->tag instanceof CompoundTag){
            if(self::$LTIteminitd and Main::isThisItemForNBT($tag->tag) and ($data=Main::getTypeAndNameForNBT($tag->tag))!==false and ($conf=Main::getInstance()->getConf($data[0], $data[1]))!==false){
                switch($data[0]){
                    case '近战':
                    case '远程':
                    case '通用':
                        return Weapon::getWeapon($data[0], $conf, $tag->Count->getValue(), $tag->tag);
                        break;
                    case '盔甲':
                    ...
                    case '材料':
                        ...
                        break;
                    case '魔法':
                        return ManaSystem::getManaItem($data[1], $conf, $tag->Count->getValue(), $tag->tag);
                        break;
                    case '饰品':
                        return new BaseOrnaments($conf, $tag->Count->getValue(), $tag->tag);
                        break;
                }
            }
        }
