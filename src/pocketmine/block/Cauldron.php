...
class Cauldron extends Transparent{
	...
	public function onActivate(Item $item, Player $player = null){//@author iTX. rewrite @Dog194
		...
		if ($tile->getCompleteManyBlockStructure() and $this->isFull() and $player!=null and $item->getId()!=Item::BUCKET and $player->isSurvival()){
		    $count = $item->getCount();
		    if ($count > 1){
		        $count--;
		        $item->setCount($count);
                ...
                $item = clone $item;//见鬼..
                $item->setCount(1);
                $this->getLevel()->dropItem($this->add(0.5, 1, 0.5), $item, new Vector3(0, 0,0));
            }elseif($count == 1){
            ...
            switch($item->getId()){
                ...
                case Item::SPLASH_POTION:
                    if($player instanceof Player and ($player->isOp() or $player->isCreative()))return true;
