...
class ItemFrame extends Flowable {
	...
	public function onActivate(Item $item, Player $player = null){
		...
		if($tile->hasItem()){
			...
			if($player instanceof Player and $item->getCount() > 0 and $player->isSurvival()){
				...
				if ($this->getSide($sides[$this->meta]) instanceof Glass and $frameItem instanceof Drawings){//有可能是熔炼坛
                    if (($entity = Fusion::tryCreate($this, $frameItem, $player))==null){//满足多方块结构
                        $player->sendMessage("§c§l多方块结构有误，清仔细排查后再次尝试吧！");
                    }
                }
			}
		}
