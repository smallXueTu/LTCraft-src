...
class SessionManager{
	...
	protected $lastRespond = PHP_INT_MAX;
	...
	private function tick(){
        ...
        // if(time() - $this->lastRespond>3){
			// $this->server->addEventQueue('HEA');
		// }
        // if (time() - $this->lastRespond>10 and $this->send == false){
			// $text = '';
            // while(strlen($packet = $this->server->getSTAQueue()) > 0){
               // $text.=$packet. PHP_EOL;
            // }
			// file_put_contents('/root/STA.txt', $text, FILE_APPEND);
			// $this->send = true;
            // file_get_contents('http://down.ltcraft.cn:88/CoolQ?key=266533&type=group&group=827169569&mess='.'[CQ:at,qq=all]注意！服务器10没有回应心跳服务器可能已卡死！');
        // }elseif(time() - $this->lastRespond>60){
	...
	private function receivePacket(){
		...
		if($buffer !== null){
			...
			if(substr($buffer,0,3)==='WED'){
				$buffer=substr($buffer,3,strlen($buffer));
				if(in_array(substr($buffer,0,3),['GET', 'QQS']))
					$this->server->addEventQueue($buffer.'>'.$source.':'.$port);
				elseif(in_array(substr($buffer,0,3),['SAY', 'ASY', 'USY']))
					$this->server->addEventQueue($buffer);
				return true;
			}
	...
	public function receiveStream(){
		if(strlen($packet = $this->server->readMainToThreadPacket()) > 0){
			if(substr($packet,0,3)==='WED'){
				$packet=substr($packet,3,strlen($packet));
				$pk=explode('€',$packet);
				if($pk[0]=='setip'){
					$this->ip = $pk[1];
					return;
				...
				$to=explode(':',$pk[1]);
				$this->socket->writePacket($pk[0], $to[0], $to[1]);
			}
			// elseif(substr($packet,0,3)==='HEA'){
	...
	public function removeSession(Session $session, $reason = "未知"){
		$id = $session->getAddress() . ":" . $session->getPort();
		if(isset($this->sessions[$id])){
