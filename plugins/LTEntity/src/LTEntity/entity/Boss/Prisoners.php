...
class Prisoners extends ANPC
{
    ...
    public function onUpdate($currentTick)
    {
        ...
        if ($this->nullTargetTick > 20 * 5 and $this->age % 20 == 0 and !($this->baseTarget instanceof Player) and $this->getHealth() < $this->getMaxHealth()){
    ...
    public function attack($damage, EntityDamageEvent $source)
    {
        if ($source instanceof EntityDamageByEntityEvent and $source->getDamager() instanceof Player){
            ...
            if (Main::getKillNumber('囚禁者', $damager->getName()) >= 5){
                $damager->sendCenterTip('§c你今天击杀囚禁者已达上线§e5§c了！');
                $source->setCancelled();
                return false;
            }
            if (time() - Main::getLastKillerTime('囚禁者', $damager->getName()) < 1000){
                $d = Main::getLastKillerTime('囚禁者', $damager->getName()) + 1000 - time();
                $damager->sendCenterTip('§c你还需等待'.$d.'秒才可以再次对囚禁者发起单挑。');
                $source->setCancelled();
                return false;
            }
            $damage = 3000;
