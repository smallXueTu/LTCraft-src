...
class Converter
{
	public static function getPngSkin($path,$useCache=true)
	{
		try
		{
			if($useCache && is_file(Main::getInstance()->getDataFolder().'skins/cache/'.basename($path).'.cache'))
			{
				return file_get_contents(Main::getInstance()->getDataFolder().'skins/cache/'.basename($path).'.cache');
			}
			if(!is_file($path))
			{
				return -1;
			...
			unset($png,$useCache);
			file_put_contents(Main::getInstance()->getDataFolder().'skins/cache/'.basename($path).'.cache',$skin);
			return $skin;
		}
		catch(\Exception $e)
		{
	...
	public static function PPM2Skin($ppm)
	{
		...
		{
			Main::getInstance()->getLogger()->info(\pocketmine\utils\TextFormat::RED.'错误的PPM图片,仅支持Binary格式');
			unset($ppm);
			return false;
		}
