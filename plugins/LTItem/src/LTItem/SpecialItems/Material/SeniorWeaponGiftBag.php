...
class SeniorWeaponGiftBag extends Material
{
    /**
     * @param Level $level
     * @param Player $player
        ...
        if($level->getName()=='pve' and $target->getX()==-287 and $target->getY()==102 and $target->getZ()==455) {
            ...
            switch(true){
                ...
                case $rand>=70:
                    ...
                    break;
                case $rand>=40:
                    $item=Main::getInstance()->createWeapon('近战', '水晶之痕', $player);
                    break;
                case $rand>=20:
                    $item=Main::getInstance()->createWeapon('近战', '欲火刃', $player);
                    break;
                case $rand>=0:
                    $item=Main::getInstance()->createWeapon('近战', '冰雪手斧', $player);
                    break;
            }
            $this->count--;
            $player->getInventory()->addItem($item);
            $player->sendMessage('§e§l哇 你打开高级武器礼包获得了:'.$item->getLTName());
