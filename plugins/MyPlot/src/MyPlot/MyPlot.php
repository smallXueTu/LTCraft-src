...
class MyPlot extends PluginBase implements Listener
{
     ...
     * @return true|false
     */
	public function canUseRoad(Position $position, Player $player){
        $x = $position->x;
        $z = $position->z;
        ...
        if ($plotLevel === null) {
            return false;//这个世界没有地皮设置 不是地皮世界？
        }
		
		if($player->hasPermission("myplot.admin.build.road")){
			return true;//玩家是OP
		}
		
        $plotSize = $plotLevel->plotSize;//地皮大小
        $roadWidth = $plotLevel->roadWidth;//路面宽度
        $totalSize = $plotSize + $roadWidth;//总大小
		...
		$accord = 0;
		$need = (($difZ > $plotSize - 1) and ($difX > $plotSize - 1))?4:2;
		$username = strtolower($player->getName());
        if (($difX > $plotSize - 1)) {//X轴在路面 接下来判断路两边地皮是否为玩家自己的！
			if ($x >= 0) {//正 最大为左边
				$XP1 = $this->dataProvider->getPlot($levelName, $X, $Z);
				$XP2 = $this->dataProvider->getPlot($levelName, $X+1, $Z);
				if (($XP1->owner == $username or $XP1->isHelper($username))){
					$accord++;
				}
				if (($XP2->owner == $username or $XP2->isHelper($username))){
					$accord++;
				}
			}else{
				$XP1 = $this->dataProvider->getPlot($levelName, $X, $Z);
        ...
        if (($difZ > $plotSize - 1)) {//Z轴在路面 接下来判断路两边地皮是否为玩家自己的！
			if ($z >= 0) {//正 最大为左边
				$XP1 = $this->dataProvider->getPlot($levelName, $X, $Z);
				$XP2 = $this->dataProvider->getPlot($levelName, $X, $Z+1);
				if (($XP1->owner == $username or $XP1->isHelper($username))){
					$accord++;
				}
				if (($XP2->owner == $username or $XP2->isHelper($username))){
    ...
    public function onEnable() {
       // $this->getLogger()->info("Loading MyPlot");
		...
		$this->additionalPlot=new Config($this->getDataFolder().'additionalPlot.yml',Config::YAML,[]);
