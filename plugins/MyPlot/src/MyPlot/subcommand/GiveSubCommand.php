...
class GiveSubCommand extends SubCommand
{
    ...
    public function execute(CommandSender $sender, array $args) {
        ...
        if (!($newOwner instanceof Player)) {
            ...
            return true;
        } elseif (strtolower($newOwner->getName()) === $plot->owner) {
            $sender->sendMessage(TextFormat::RED . $this->translateString("give.toself"));
            return true;
        }

        $maxPlots = $newOwner->isOp()?8:4;
		$maxPlots+=$this->getPlugin()->additionalPlot->get(strtolower($newOwner->getName()),0);
        ...
        if ($confirm) {
            $plot->owner = strtolower($newOwner->getName());
            $plot->ownerName = $newOwner->getName();
