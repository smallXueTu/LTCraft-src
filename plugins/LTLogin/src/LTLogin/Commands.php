...
class Commands{
	...
	public function onCommand($Sender,$cmd,$label,$args){
		...
		switch(strtolower($args[0])){
			case 'email':
				if(isset($this->event->datas[$name]['email']) and $this->event->datas[$name]['email']!==NULL)return $Sender->sendMessage('§l§a[提示]§c你已经绑过邮箱了');return;
				if(!isset($args[1]))return $Sender->sendMessage('§l§a[提示]§c用法：§d/login email §e邮箱地址');
				if(preg_match('/^([0-9A-Za-z\\-_\\.]+)@([0-9a-z]+\\.[a-z]{2,3}(\\.[a-z]{2})?)$/i',$args[1])){
					...
					$Sender->sendMessage('§l§a[提示]§a绑定成功,你以后可以通过这个来找回密码　 &&　开发中．．');
				}else{
					$Sender->sendMessage('§l§a[提示]§c邮箱地址不合法');
				}
			break;
			case 'qq':
				if(isset($this->event->datas[$name]['qq']) and $this->event->datas[$name]['qq']!==NULL)return $Sender->sendMessage('§l§a[提示]§c你已经绑过QQ了');return;
				if(!isset($args[1]))return $Sender->sendMessage('§l§a[提示]§c用法：§d/login qq QQ号');
				Server::getInstance()->dataBase->pushService('1'.chr(2)."update user set QQ='{$args[1]}' where name='{$name}'");
				$Sender->sendMessage('§l§a[提示]§a绑定成功,你以后可以通过这个来找回密码 或查询');
			break;
			case 'resetp':
				if(!isset($args[1])){$Sender->sendMessage('§l§a[提示]§c用法：§d/login resetp §e新密码');return;}
				...
				// if($code!=false){
					...
					// if($code['outtime']>=time()){
						// if($code['code']==$args[1]){
							...
							if($check!==true){
								$Sender->sendMessage('§l§a[提示]§c'.$check);
								return;
							}
			...
			break;
			default:
				$Sender->sendMessage('§l§a[提示]§c未知命令 输入§a/login help §c获取帮助列表');
			break;
		}
	}
}